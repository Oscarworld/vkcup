...
final class GroupsViewController: UIViewController {
    ...
    private lazy var titleButton: UIButton = {
        $0.backgroundColor = UIColor.Styles.white
        $0.setImage(UIImage.Styles.dropdownFlipped, for: .normal)
        $0.imageEdgeInsets = .init(top: 0, left: 4, bottom: 0, right: 0)
        $0.semanticContentAttribute = .forceRightToLeft
        $0.addTarget(self, action: #selector(didPressTitleButton), for: .touchUpInside)
        return $0
    }(UIButton())
    ...
    @objc
    private func didPressTitleButton() {
        output.didPressCities()
    }
}
...
extension GroupsViewController: GroupsViewInput {
    ...
    func showPlaceholder(
        _ state: GroupsPlaceholderState
    ) {
        switch state {
        case .emptyList:
        ...
        case .failFetchCities:
            placeholderView.title = Localized.Placeholder.FailFetchCities.title
            placeholderView.subtitle = Localized.Placeholder.FailFetchCities.subtitle
            placeholderView.action = { [weak self] in
                self?.output.didPressReloadCities()
            }
        }
...
// MARK: - GroupsTransitionHandler

extension GroupsViewController: GroupsTransitionHandler { }
...
extension GroupsViewController: UIScrollViewDelegate {
...
private extension GroupsViewController {
    enum Localized {
        ...
        static let screenTitle = NSLocalizedString(
            "GroupsViewController.ScreenTitle",
            value: "Документы",
            comment: "Title на экране Документов"
        )
        ...
        enum Placeholder {
            ...
            enum EmptyState {
                ...
                static let subtitle = NSLocalizedString(
                    ...
                    value: "Чтобы здесь появились группы, необходима сначала их добавить",
            ...
            enum FailFetchGroups {
                static let title = NSLocalizedString(
                    ...
                    value: "Ошибка при получения групп",
            ...
            enum FailFetchCities {
                static let title = NSLocalizedString(
                    "GroupsViewController.Placeholder.FailFetchCities.Title",
                    value: "Ошибка при получения городов",
                    comment: "Title в Header на экране групп при ошибки получения городов"
                )
                static let subtitle = NSLocalizedString(
                    "GroupsViewController.Placeholder.FailFetchCities.Subtitle",
                    value: "Попробуй повторить запрос",
                    comment: "Subtitle в Header на экране групп при ошибки получения данных городов"
                )
            }
        }
